...
func TestUnitMapper(t *testing.T) {
	...
	Convey("test CreatePreviewPage correctly maps to model.Preview", t, func() {
		...
		pp := CreatePreviewPage(req, bp, dimensions, testFilter, testDataset, testFilter.FilterID, "12345", "11-11-1992", "/v1", false, "en", serviceMessage, emergencyBanner)
		So(pp.SearchDisabled, ShouldBeFalse)
		So(pp.Breadcrumb, ShouldHaveLength, 4)
		So(pp.Breadcrumb[0].Title, ShouldEqual, testDataset.Title)
		...
		So(pp.Breadcrumb[2].URI, ShouldEqual, "/filters/"+testFilter.FilterID+"/dimensions")
		So(pp.Breadcrumb[3].Title, ShouldEqual, "Preview")
		So(pp.Breadcrumb[3].URI, ShouldEqual, "")
		So(pp.Data.FilterID, ShouldEqual, testFilter.Links.FilterBlueprint.ID)
	...
	Convey("test CreateListSelector page... ", t, func() {
		Convey("correctly maps to model.Selector", func() {
			...
			p := CreateListSelectorPage(req, bp, "time", selectedValues, allValues, testFilter, d, dataset.VersionDimensions{}, "12345", "/v1", "en", serviceMessage, emergencyBanner)
			So(p.Data.Title, ShouldEqual, "Time")
			So(p.SearchDisabled, ShouldBeTrue)
			So(p.FilterID, ShouldEqual, testFilter.FilterID)
			...
			So(p.Breadcrumb[2].URI, ShouldEqual, "/filters/"+testFilter.Links.FilterBlueprint.ID+"/dimensions")
			...
			So(p.Data.AddFromRange.URL, ShouldEqual, "/filters/"+testFilter.FilterID+"/dimensions/time")
			So(p.Data.SaveAndReturn.URL, ShouldEqual, "/filters/"+testFilter.FilterID+"/dimensions")
			So(p.Data.Cancel.URL, ShouldEqual, "/filters/"+testFilter.FilterID+"/dimensions")
			So(p.Data.AddAllInRange.Label, ShouldEqual, "All times")
			So(p.Data.RangeData.URL, ShouldEqual, "/filters/"+testFilter.FilterID+"/dimensions/time/list")
			So(p.Data.RemoveAll.URL, ShouldEqual, "/filters/"+testFilter.FilterID+"/dimensions/time/remove-all")
